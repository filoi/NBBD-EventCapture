...
.controller('MainController',
    ...
    var eventIdFromUrl = ($location.search()).event;
            ...
            $scope.popupCenter = function (url, title) {

                return window.open(url, title, "width=1330,height=500");
            };

            //CUSTOM FUNCTION
            $scope.selectimage = function (_opt, _co) {
                $scope.opt = _opt;
                $scope.co = _co;
                $("#myModalhouse").modal('show');
                $("#myModalhouse").on('hidden.bs.modal', function () {
                    $scope.bind_value($scope.opt, $scope.co);
                    $scope.saveDatavalue();
                });
            };

            $scope.bind_value = function (opt_recive, co_recieve) {
                $scope.optt = opt_recive;
                $scope.codo = co_recieve;
                var imaagedata = localStorage.getItem("firstname");
                if (imaagedata !== "blank") {
                    var params = imaagedata.split(',');
                    var code = params[0];
                    var type_birth = params[1];
                    var type_birth_value = type_birth.slice(0, -1);

                    $scope.currentEvent[$scope.optt] = type_birth_value;
                    $scope.currentEvent[$scope.codo] = code + ")";

                    var imaagedata_index = localStorage.getItem("imageindesvalue");
                    document.getElementById(imaagedata_index).src = 'image-picker/circle.png';
                    localStorage.setItem("firstname", "blank");
                    localStorage.setItem("imageindesvalue", "blank");
                }
                /* else{
            ...
            $scope.custom_fuction = function () {

                $scope.optt_array = ["PXMJEsHKsn9", "RKjZskDwagM", "p7E2JhTzYOx", "kec8K0ZD9PE", "HDtbM8sgZej", "cZTSYZzP2oA", "hcUo6yeGCd2", "dK0a7yjNjig", "scXVJgWywrv", "R0M1X9tnmQu"];

                $scope.codo = "";
                for (var h = 0; h < $scope.optt_array.length; h++) {
                    var checkvalue = $scope.currentEvent[$scope.optt_array[h]];
                    if ($scope.optt_array[h] == "PXMJEsHKsn9") $scope.codo = "lPLNNrQTkEb";else if ($scope.optt_array[h] == "RKjZskDwagM") $scope.codo = "ygEjJiJgCke";else if ($scope.optt_array[h] == "p7E2JhTzYOx") $scope.codo = "rqHwZlH9JBY";else if ($scope.optt_array[h] == "kec8K0ZD9PE") $scope.codo = "HRv7WpU5WLO";else if ($scope.optt_array[h] == "HDtbM8sgZej") $scope.codo = "i37IlR6MfJr";else if ($scope.optt_array[h] == "cZTSYZzP2oA") $scope.codo = "jrdrTSsvqz8";else if ($scope.optt_array[h] == "hcUo6yeGCd2") $scope.codo = "C3h09TXYoNu";else if ($scope.optt_array[h] == "dK0a7yjNjig") $scope.codo = "tzfm6dtpzO4";else if ($scope.optt_array[h] == "scXVJgWywrv") $scope.codo = "XMZXz5auRMl";else if ($scope.optt_array[h] == "R0M1X9tnmQu") $scope.codo = "H38uvKSWnU8";
                    if (checkvalue !== "" && checkvalue !== null) {
                        if (checkvalue == "Absence or hypoplasia of first toe with other digits present (Q72.31)") $scope.currentEvent[$scope.codo] = "(Q72.31)";else if (checkvalue == "Absence or hypoplasia of thumb (other digits intact)(Q71.31)") $scope.currentEvent[$scope.codo] = "(Q71.31)";else if (checkvalue == "Anencephaly (Q00.0)") $scope.currentEvent[$scope.codo] = "(Q00.0)";else if (checkvalue == "Cervical spina bifida with hydrocephalus (Q05.0)") $scope.currentEvent[$scope.codo] = "(Q05.0)";else if (checkvalue == "Cervical spina bifida without hydrocephalus (Q05.5)") $scope.currentEvent[$scope.codo] = "(Q05.5)";else if (checkvalue == "Cleft hard palate with bilateral cleft lip (Q37.0)") $scope.currentEvent[$scope.codo] = "(Q37.0)";else if (checkvalue == "Cleft hard palate with cleft lip, specified as unilateral (Q37.10)") $scope.currentEvent[$scope.codo] = "(Q37.10)";else if (checkvalue == "Cleft lip, bilateral (Q36.0)") $scope.currentEvent[$scope.codo] = "(Q36.0)";else if (checkvalue == "Cleft lip, specified as unilateral (Q36.9, Q36.90)") $scope.currentEvent[$scope.codo] = "(Q36.9, Q36.90)";else if (checkvalue == "Cleft palate (Q35.5)") $scope.currentEvent[$scope.codo] = "(Q35.5)";else if (checkvalue == "Congenital absence of both forearm and hand (Q71.2)") $scope.currentEvent[$scope.codo] = "(Q71.2)";else if (checkvalue == "Congenital absence of both lower leg and foot (Q72.2)") $scope.currentEvent[$scope.codo] = "(Q72.2)";else if (checkvalue == "Congenital absence of finger(s)(remainder of hand intact)(Q71.30)") $scope.currentEvent[$scope.codo] = "(Q71.30)";else if (checkvalue == "Congenital absence of foot and toe(s) (Q72.3)") $scope.currentEvent[$scope.codo] = "(Q72.3)";else if (checkvalue == "Congenital absence of hand and finger(s)(Q71.3)") $scope.currentEvent[$scope.codo] = "(Q71.3)";else if (checkvalue == "Congenital absence of thigh and lower leg with foot present; phocomelia of lower limb (Q72.1)") $scope.currentEvent[$scope.codo] = "(Q72.1)";else if (checkvalue == "Congenital absence of upper arm and forearm with hand present; phocomelia of upper limb (Q71.1)") $scope.currentEvent[$scope.codo] = "(Q71.1)";else if (checkvalue == "Congenital complete absence of lower limb(s); amelia of lower limb (Q72.0)") $scope.currentEvent[$scope.codo] = "(Q72.0)";else if (checkvalue == "Congenital complete absence of upper limb(s); amelia of upper limb (Q71.0)") $scope.currentEvent[$scope.codo] = "(Q71.0)";else if (checkvalue == "Craniorachischisis (Q00.1)") $scope.currentEvent[$scope.codo] = "(Q00.1)";else if (checkvalue == "Exomphalos/omphalocele (Q79.2)") $scope.currentEvent[$scope.codo] = "(Q79.2)";else if (checkvalue == "Frontal encephalocele (Q01.0)") $scope.currentEvent[$scope.codo] = "(Q01.0)";else if (checkvalue == "Gastroschisis (Q79.3)") $scope.currentEvent[$scope.codo] = "(Q79.3)";else if (checkvalue == "Hypospadias (Q54)") $scope.currentEvent[$scope.codo] = "(Q54)";else if (checkvalue == "Hypospadias (Q54.0)") $scope.currentEvent[$scope.codo] = "(Q54.0)";else if (checkvalue == "Hypospadias (Q54.1)") $scope.currentEvent[$scope.codo] = "(Q54.1)";else if (checkvalue == "Hypospadias (Q54.2)") $scope.currentEvent[$scope.codo] = "(Q54.2)";else if (checkvalue == "Hypospadias (Q54.3)") $scope.currentEvent[$scope.codo] = "(Q54.3)";else if (checkvalue == "Hypospadias (Q54.9)") $scope.currentEvent[$scope.codo] = "(Q54.9)";else if (checkvalue == "Imperforate Anus (42.3)") $scope.currentEvent[$scope.codo] = "(Q42.3)";else if (checkvalue == "Iniencephaly (Q00.2)") $scope.currentEvent[$scope.codo] = "(Q00.2)";else if (checkvalue == "Longitudinal reduction defect of femur (Q72.4)") $scope.currentEvent[$scope.codo] = "(Q72.4)";else if (checkvalue == "Longitudinal reduction defect of fibula; fibular aplasia/hypoplasia (Q72.6)") $scope.currentEvent[$scope.codo] = "(Q72.6)";else if (checkvalue == "Longitudinal reduction defect of radius: clubhand (congenital), radial clubhand, absence of radius (Q71.4)") $scope.currentEvent[$scope.codo] = "(Q71.4)";else if (checkvalue == "Longitudinal reduction defect of tibia (Q72.5)") $scope.currentEvent[$scope.codo] = "(Q72.5)";else if (checkvalue == "Longitudinal reduction defect of ulna (Q71.5)") $scope.currentEvent[$scope.codo] = "(Q71.5)";else if (checkvalue == "Lumbar spina bifida with hypdrocephalus (Q05.2)") $scope.currentEvent[$scope.codo] = "(Q05.2)";else if (checkvalue == "Lumbar spina bifida without hydrocephalus (Q05.7)") $scope.currentEvent[$scope.codo] = "(Q05.7)";else if (checkvalue == "Nasal encephalocele (Q01.82)") $scope.currentEvent[$scope.codo] = "(Q01.82)";else if (checkvalue == "Nasofrontal Encephalocele (Q01.1)") $scope.currentEvent[$scope.codo] = "(Q01.1)";else if (checkvalue == "Occipital encephalocele (Q01.2)") $scope.currentEvent[$scope.codo] = "(Q01.2)";else if (checkvalue == "Orbital encephalocele (Q01.81)") $scope.currentEvent[$scope.codo] = "(Q01.81)";else if (checkvalue == "Other Hypospadias (Q54.8)") $scope.currentEvent[$scope.codo] = "(Q54.8)";else if (checkvalue == "Others") $scope.currentEvent[$scope.codo] = "(Others)";else if (checkvalue == "Parietal encephalocele (Q01.80)") $scope.currentEvent[$scope.codo] = "(Q01.80)";else if (checkvalue == "Sacral spina bifida with hydrocephalus (Q05.3)") $scope.currentEvent[$scope.codo] = "(Q05.3)";else if (checkvalue == "Sacral spina bifida without hydrocephalus (Q05.8)") $scope.currentEvent[$scope.codo] = "(Q05.8)";else if (checkvalue == "Split foot (congenital cleft foot)(Q72.7)") $scope.currentEvent[$scope.codo] = "(Q72.7)";else if (checkvalue == "Split hand (congenital cleft hand)(Q71.6)") $scope.currentEvent[$scope.codo] = "(Q71.6)";else if (checkvalue == "Talipes equinovarus (Q66.0)") $scope.currentEvent[$scope.codo] = "(Q66.0)";else if (checkvalue == "Thoracic spina bifida with hydrocephalus ((Q05.1)") $scope.currentEvent[$scope.codo] = "(Q05.1)";else if (checkvalue == "Thoracic spina bifida without hydrocephalus (Q05.6)") $scope.currentEvent[$scope.codo] = "(Q05.6)";
                    }
                }
                //$scope.saveDatavalue();
            };
    ...
    $scope.generateCode = function(de) {
        ...
        OrgUnitFactory.getFromStoreOrServer($scope.selectedOrgUnit.id).then(function (orgUnitFromStore) {
            if(orgUnitFromStore) {
                var strArr = orgUnitFromStore.path.split("/");
                console.log("length is: "+strArr.length);
                codeName = getOrgUnitCode(strArr[1]).code;
                for (var i = 2; i <= strArr.length-1; i++) {
                   console.log("strArr is: "+strArr[i]);
                   var next = getOrgUnitCode(strArr[i]).code;
                   codeName = codeName + "-" + next;
                }

                codeName = codeName+"-"+(orgUnitName.substring(0, 4))+"-"+year ;
                codeName = codeName + ("000" + (list + 1)).slice(-4);
            }
        });
    ...
    function getOrgUnitCode(uid){
        ...
        $.ajax({
            ...
            url: "../api/organisationUnits/" + uid + ".json?fields=id,displayName,code&paging=false",
            success: function (responseName) {
                $scope.orgUnit = responseName;
            }
        });
